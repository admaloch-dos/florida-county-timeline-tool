...
import {
  ...
  updateCountyTimeline,
} from "../modules/addCountyPeriodItems.js";
...
import {
  resetCurrCounty,
  updateCurrTimeline,
} from "../modules/resetCurrCounty.js";
...
setTimeout(() => {
  if (checkIfTouchScreen()) {
    document
      .querySelector('[data-id="county-select"]')
...
countySelect.addEventListener("change", () => {
  document
    .querySelector(".bootstrap-select")
...
document.querySelectorAll(".active-arrows").forEach((arrow) => {
  ...
  arrow.addEventListener("click", () => {
    ...
    data.yearIndex =
      parseInt(document.querySelectorAll(".thumb-map-img")[0].id) - 1;
    ...
    if (
      arrow.id === "next-thumb-arrow" &&
      thumbImages[thumbImages.length - 1].id !== "21"
    ) {
...
const affiliatedSites = document.getElementById("AffilatedSites");

function handleResize() {
  const isShowing = affiliatedSites.classList.contains("show"); // Check if it's currently showing
  if (isShowing) {
    if (window.innerWidth < 992) {
      affiliatedSites.classList.add("collapse");
      affiliatedSites.classList.remove("show");
    }
  }
}
